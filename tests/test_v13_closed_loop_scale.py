"""Focused tests for V13-G Closed-Loop Scale V2.

Defaults use reduced candidate counts for CI speed. Full 10000/5000 targets
are exercised by ``tools/research/run_v13_closed_loop_scale.py``.
"""
...
from backend.nexus_system.closed_loop_scale_v13 import (  # noqa: E402
    CANONICAL_PATH,
    FROZEN_SEED,
    HARD_BANS,
    PASS_STATUS,
...
OWNED = (
    REPO_ROOT / "backend/nexus_system/closed_loop_scale_v13",
    REPO_ROOT / "tools/research/run_v13_closed_loop_scale.py",
    REPO_ROOT / "tests/test_v13_closed_loop_scale.py",
)
...
def test_targets_and_exports() -> None:
    assert TARGET_CANDIDATES == 10_000
    assert TARGET_COMPLETED_LIFECYCLES == 5_000
    assert PASS_STATUS.startswith("NEXUS_V13_G")
...
def test_universe_multi_symbol_multi_regime() -> None:
    cands = build_scale_candidates(120, seed=FROZEN_SEED, complete_quota=70)
    uni = universe_summary(cands)
    assert uni["symbol_count"] >= 4
...
def test_cancel_replace_and_qualification_probes() -> None:
...
def test_smoke_campaign_two_pass(tmp_path: Path) -> None:
    a = run_v13_closed_loop_scale_campaign(
        root=tmp_path / "pass1",
        candidate_count=24,
        seed=FROZEN_SEED,
        keep_root=True,
    ...
    b = run_v13_closed_loop_scale_campaign(
        root=tmp_path / "pass2",
        candidate_count=24,
        seed=FROZEN_SEED,
        keep_root=True,
    ...
    assert a["universe"]["symbol_count"] >= 4
...
def test_negative_false_pass_blocked_by_exchange_write(tmp_path: Path) -> None:
