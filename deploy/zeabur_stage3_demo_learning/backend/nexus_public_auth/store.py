...
class PublicAccount:
    ...
    # Password credential — PBKDF2 hash only; never store plaintext.
    password_hash: Optional[str] = None
    email_verified: bool = False
    email_verified_at: Optional[str] = None
    failed_login_count: int = 0
    lockout_until_epoch: Optional[float] = None
...
class PublicAuthStore:
    ...
    def create_account(
        ...
        *,
        ...
        if not normalized or "@" not in normalized:
            raise HardBanViolation("valid email required")
        with self._lock:
            if normalized in self.accounts_by_email:
                raise HardBanViolation("email already registered")
            account = PublicAccount(
                ...
                email_verified_at=_utcnow() if email_verified else None,
            )
            self.accounts[account.account_id] = account
            self.accounts_by_email[normalized] = account.account_id
    ...
    def get_account_by_email(self, email: str) -> Optional[PublicAccount]:
        with self._lock:
            aid = self.accounts_by_email.get((email or "").strip().lower())
            if not aid:
                return None
    ...
    def update_account(self, account: PublicAccount) -> PublicAccount:
        ...
        with self._lock:
            prev = self.accounts.get(account.account_id)
            if prev and prev.email != account.email:
                self.accounts_by_email.pop(prev.email, None)
            self.accounts[account.account_id] = account
            self.accounts_by_email[account.email] = account.account_id
        return deepcopy(account)
