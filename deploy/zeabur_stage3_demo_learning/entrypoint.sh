  ...
  if [ "${STAGE4_DRY_RUN_ONLY:-false}" = "true" ] && [ "${STAGE4_CLOUD_DRY_RUN_MINUTES:-0}" != "0" ]; then
    ...
    if [ "${STAGE4_REQUIRE_REAL_LLM:-false}" = "true" ]; then
      if ! python tools/research/run_stage4_ai_decision_dry_run.py         --preflight-only         --use-real-llm         --output-dir "$STAGE4_OUT"         >> "$STAGE4_OUT/stage4_cloud_dry_run.log" 2>&1; then
        ...
        python tools/research/run_stage4_ai_decision_dry_run.py           --duration-minutes "${STAGE4_CLOUD_DRY_RUN_MINUTES}"           --poll-interval-seconds "${STAGE4_POLL_INTERVAL_SECONDS:-120}"           --symbols "${STAGE4_SYMBOLS:-ETHUSDT,BTCUSDT}"           --mode dry-run           --use-real-llm           --output-dir "$STAGE4_OUT"           >> "$STAGE4_OUT/stage4_cloud_dry_run.log" 2>&1 &
      fi
    else
      echo "Stage 4 cloud dry-run: ${STAGE4_CLOUD_DRY_RUN_MINUTES}m -> $STAGE4_OUT (background, no orders)"
      python tools/research/run_stage4_ai_decision_dry_run.py         --duration-minutes "${STAGE4_CLOUD_DRY_RUN_MINUTES}"         --poll-interval-seconds 60         --symbols "${STAGE4_SYMBOLS:-ETHUSDT,BTCUSDT}"         --mode dry-run         --use-real-llm         --output-dir "$STAGE4_OUT"         >> "$STAGE4_OUT/stage4_cloud_dry_run.log" 2>&1 &
    fi
  fi
fi
