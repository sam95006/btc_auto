...
if [ "$MODE" = "idle" ]; then
  ...
  if [ "${STAGE4_DRY_RUN_ONLY:-false}" = "true" ] && [ "${STAGE4_CLOUD_DRY_RUN_MINUTES:-0}" != "0" ]; then
    ...
    STAGE4_SYMBOL_LIST="${STAGE4_SYMBOLS:-ETHUSDT,BTCUSDT}"
    mkdir -p "$STAGE4_OUT"
    if [ "${STAGE4_REQUIRE_REAL_LLM:-false}" = "true" ]; then
      if ! python tools/research/run_stage4_ai_decision_dry_run.py \
        ...
        echo "Stage 4 cloud dry-run: ${STAGE4_CLOUD_DRY_RUN_MINUTES}m symbols=${STAGE4_SYMBOL_LIST} -> $STAGE4_OUT (background, no orders)"
