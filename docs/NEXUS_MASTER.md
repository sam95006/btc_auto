...
## 雲端部署（Zeabur）與本地／幣安資料一致

**唯一維護的部署說明**：[`tools/deploy/ZEABUR_SETUP.zh-TW.md`](../tools/deploy/ZEABUR_SETUP.zh-TW.md)

以下與目前 **NEXUS** 程式一致（舊版 `ZEABUR_DEPLOYMENT`／`storage.py`／`/app/data` 自動備份等敘述已過時並自本文件移除，避免與實作衝突）。

| 項目 | 說明 |
|------|------|
| 幣安 Testnet | Zeabur「變數」須與本地 `.env` **鍵名與值一致**（四把 `BINANCE_*_TESTNET_*`、`NEXUS_TRADING_MODE` 等）；`.env` 不會進 Git。 |
| 執行緒 | 單一 Zeabur 服務時會偵測 `ZEABUR_*` 並**內嵌 worker**；若另開獨立 worker，請在 web 設 `NEXUS_WEB_ONLY=1`。 |
| 資料庫與版面 | 設 `NEXUS_DATA_DIR`（掛 Volume，例如 `/data`）+ `NEXUS_RUNTIME_DB=trading.db`；與本地同一路徑語意可避免重啟後「銜接斷裂」。 |
| 狀態匯出 | `python tools/deploy/nexus_state_sync.py export`，再將 zip 解至雲端 `NEXUS_DATA_DIR`。 |
| 變數自查 | `python tools/deploy/check_env_parity.py`（只印 SET/MISSING，不印金鑰）。 |
| HTTP 診斷 | 部署後開 `GET /api/nexus/connectivity`（不含機密）。 |

若 Zeabur 儀表板顯示「工作節點離線」、資金全零，優先檢查：**變數是否齊全**、**是否僅有 web 而無內嵌／獨立 worker**、**持久卷是否掛上**。
