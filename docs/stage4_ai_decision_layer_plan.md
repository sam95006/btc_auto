...
- **4.18o:** BTC-specific schema/sample diagnostics — offline N-R2 replay; code-only; **PASS** (`btc_valid_watch=0`, 12/12 soft_skip Groq, primary cause=no-edge skip); **Stage 4.19 remains blocked** until BTC + ETH graduation >0; **no soak**.
- **4.18o2:** Provider routing diagnostics — offline N-R2 replay; **PASS** (`routing_asymmetry_detected=true`, BTC 100% Groq / ETH 100% Cerebras via fallback, all valid_watch from Cerebras); recommend **4.18-O3** controlled probe design only; **Stage 4.19 remains blocked**; **no soak**.
- **4.18o3:** Controlled Groq-vs-Cerebras BTC provider probe — diagnostic one-shot; **PASS** (Groq 0/3 valid_watch, Cerebras 1/3 valid_watch on same frozen context; `provider_divergence_detected=true`); recommend **4.18-P** routing design gate; **Stage 4.19 remains blocked**; probe not fed to calibration.
- **4.18p:** Provider routing design gate — design-only; **PASS** (`routing_problem_confirmed=true`; recommend **Option 3** BTC dual-provider shadow → **P1**); **Stage 4.19 remains blocked** until BTC + ETH graduation >0 from non-shadow decisions; **no soak**.
- **4.18p1:** BTC dual-provider shadow implementation — code-only; **PASS** (shadow default off; separate JSONL; excluded from paper/calibration/graduation/4.19); **Stage 4.19 remains blocked**; **no soak**.
- **4.18p1-r1:** 30m BTC dual-provider shadow sample — runtime-gated; **PASS** (6/6 ticks, 20 effective, shadow JSONL×6, exclusions verified, flags reset); shadow isolation valid but **no shadow yield advantage** (shadow valid_watch=0 vs actual BTC valid_watch=1); **no P2 routing**; **Stage 4.19 remains blocked**; **60m not run**.
- **4.18p1a:** BTC shadow paired-comparison export — offline only; **PASS** (6 pairs; shadow not comparable due to quota/truncation; actual BTC watch→no graduation); **no P2 / no soak / Stage 4.19 remains blocked**.
- **4.18p1b:** Quota-aware shadow call + BTC watchlist follow-up diagnostics — code/offline; **PASS** (Groq cooldown/fallback skip; truncation retry; uncomparable classification; P1-R1 reclass comparable=1; BTC watch on last tick → no follow-up); later clean shadow sample may be justified; **no 60m now / no P2 / Stage 4.19 remains blocked**.
- **4.18p1c:** Clean 30m shadow sample + BTC follow-up gate — runtime-gated; **PASS** (6/6 ticks, 23 effective; shadow Cerebras×6; comparable 1→5; uncomparable=truncation×1 only; shadow_valid_watch=5 vs actual Groq=0; skill comparison valid but **P2 not auto-started**; actual BTC valid_watch=0 so follow-up N/A; flags reset); **Stage 4.19 remains blocked**; **60m not run**.
- **4.18p2:** Provider routing design gate — design-only; **PASS** (P1C evidence supports read-only BTC Cerebras-first experiment design; recommend Option 2 → **P2-R1** with operator approval; `routing_auto_change_allowed=false`; **P2-R1 not executed**); **Stage 4.19 remains blocked** until actual non-shadow BTC + ETH graduation >0.
- **4.18p2-r1:** BTC Cerebras-first read-only 30m experiment — runtime-gated; **PARTIAL_BTC_ONLY** (6/6 ticks, 20 effective, parse=0; BTC chain cerebras,groq; BTC provider cerebras×3/groq×2; BTC valid_watch=3; BTC graduation=3 actual-only; ETH graduation=0; flags reset; **Stage 4.19 not started**; no permanent routing).
- **4.18p2a:** ETH+BTC actual graduation alignment diagnostics — offline; **PASS** (BTC graduation=3 explained by Cerebras-first experiment; ETH decisions=6, valid_watch=1, graduation=0; root=`eth_followup_confirmation_failed`; provider still Cerebras-majority — not BTC-override poisoning; `should_run_60m=false`; **Stage 4.19 remains blocked**).
- **4.18p2b:** ETH watchlist follow-up confirmation diagnostics — offline; **PASS** (ETH watch Cerebras conf=0.55 BUY/LONG, trigger+MAE OK; follow-up same provider → hard_skip/NONE/conf=0; reason=`eth_followup_direction_changed`; recommend market-context/confirmation review; `should_run_60m=false`; **Stage 4.19 remains blocked**).
- **4.18p2c:** ETH follow-up market context / confirmation review — offline; **PASS** (watch tick 3→follow 4; price Δ≈-0.13%, regime trend→trend, trend_strength 0.41→0.64; no invalidation/MAE breach; classify=`confirmation_prompt_too_strict` / system_issue; BTC success retained LONG/BUY+trigger update vs ETH hard_skip NONE; recommend `eth_followup_confirmation_prompt_review`; `should_run_60m=false`; **Stage 4.19 remains blocked**; no prompt/schema/state-machine code change in this step).
- **4.18p2d:** ETH follow-up confirmation prompt review / repair — code-only; **PASS** (`previous_watch_context` injected into `build_decision_prompt`; direction-collapse + confidence-collapse guards; static replay `continuation_watch_or_confirmation_pending` / `would_prevent_unexplained_collapse=true`; `needs_next_runtime_regression=true`; `should_run_30m_now=false`; **Stage 4.19 remains blocked**; no MAE/RG/routing/order changes).
- **4.18p2d-r1:** ETH follow-up prompt runtime regression — 30m read-only BTC Cerebras-first experiment; **PARTIAL_NO_ETH_WATCH** (6/6 ticks, effective=18, parse=0, mock=0, order=0; BTC vw=1/grad=0 last-tick watch; ETH vw=0/grad=0 all skip; unexplained collapse N/A; flags reset; **Stage 4.19 remains blocked**; no 60m).
- **4.18p2e:** ETH no-watch diagnostics + wait helper robustness — offline; **PASS** (`sample_market_no_edge`; ETH decisions=5 all skip NONE; prompt_repair_over_conservative=false; wait helper `completed_needs_finalize` when ticks reached but dry_run_completed=false; `should_run_60m=false`; **Stage 4.19 remains blocked**; no prompt/MAE/RG/routing changes).
- **4.18p2f:** ETH watch reappearance gate + regression readiness — offline; **PASS** (`regression_readiness=false`; `do_not_run_regression_now=true`; next=`wait_for_eth_watch_conditions_reappear_no_60m`; wait helper robustness PASS; `should_run_60m=false`; **Stage 4.19 remains blocked**; no soak).
- **4.18p2g:** Operator readiness pack — offline consolidation; **PASS** (BTC prior graduation evidence exists; ETH repair done but not runtime-validated; watch gate not ready; `next_short_regression_allowed_now=false`; `should_run_30m_now=false`; `should_run_60m=false`; Stage 4.19 dossier not allowed; **Stage 4.19 remains blocked**; no soak).
- **4.18p2h:** Backend hold state + passive future ETH regression gate checker — code/docs only; **PASS** (`HOLD`; no runtime until ETH watch reappears; checker never auto-starts 30m/60m/4.19; **Stage 4.19 remains blocked**).
- **4.18p2h-ops:** Operator Hold Runbook — docs/CLI usage only; **PASS** (documents HOLD reason, future gate checker usage, short-regression approval checklist, Stage 4.19 dossier checklist; **no runtime**; next runtime only after ETH watch conditions reappear; **Stage 4.19 remains blocked**). See `docs/runbooks/STAGE_4_18_P2H_OPERATOR_HOLD_RUNBOOK.md`.
- **4.18p2h-qa:** Repository / release health check — docs/git/consistency only; **PASS** (`release_checkpoint_ready=true`; HOLD runbook/checker/UI MVP-10/README/plan consistent; no Stage 4.19 start / order / ARM / billing paths; **no runtime**; next=`hold_backend_and_continue_private_operator_ui`; **Stage 4.19 remains blocked**). See `docs/reports/STAGE_4_18P2H_QA_RELEASE_HEALTH_CHECK_REPORT.md`.
- **4.18p2h-rel:** HOLD release checkpoint — docs archive only; **PASS** (checkpoint `stage4.18-p2h-hold-checkpoint` documented; backend HOLD remains active; suggested git tag only / not auto-created; **no runtime**; next runtime only after ETH watch conditions reappear; **Stage 4.19 remains blocked**). See `docs/releases/STAGE_4_18_P2H_HOLD_RELEASE_CHECKPOINT.md`.
- **P2 current gate state:** BTC has prior actual graduation evidence; ETH prompt repair done but not runtime-validated; ETH watch reappearance gate not ready; backend HOLD; operator runbook + release health + HOLD release checkpoint archived; no 30m now; no 60m; Stage 4.19 blocked; next runtime only after ETH watch conditions reappear.
