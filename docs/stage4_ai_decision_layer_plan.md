...
- **4.18p1a:** BTC shadow paired-comparison export — offline only; **PASS** (6 pairs; shadow failures dominated by Groq `tokens` / Cerebras truncation; actual BTC watch→no graduation); recommend fix shadow provider parse/quota before more samples + optional BTC watchlist follow-up diagnostics; **no P2 / no soak / Stage 4.19 remains blocked**.
