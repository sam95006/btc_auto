#!/usr/bin/env python3
"""NEXUS UI MVP-3 ETH Confirmation Timeline safety scanner.

Checks sanitized P2B snapshot wiring under frontend/src (demo/snapshots path —
...
Requires P2B verdict + eth_followup_direction_changed displayed in frontend source.

Runnable as: python tools/research/check_nexus_ui_mvp3_safety.py
"""
...
SNAPSHOT_FILE = SNAPSHOT_DIR / "p2bPrivateOperatorSnapshot.ts"
...
REPORT_HINT = ROOT / "docs" / "ui" / "NEXUS_UI_MVP3_ETH_CONFIRMATION_TIMELINE_REPORT.md"
...
def check_snapshot_policy() -> list[str]:
    ...
    for marker in REQUIRED_SNAPSHOT_MARKERS:
        if marker not in text:
            issues.append(f"p2bPrivateOperatorSnapshot.ts missing marker: {marker}")
...
def check_mvp3_policy(files: list[Path]) -> list[str]:
    issues: list[str] = []
    ...
    if ADAPTER.is_file():
        ...
        if "p2bPrivateOperatorSnapshot" not in text:
            issues.append("adapter must import / prefer p2bPrivateOperatorSnapshot")
        if "demo/snapshots" not in text and "snapshots/p2b" not in text:
            issues.append("adapter must import from demo/snapshots (not src/data)")
        # Prefer P2B over P2A as active default
        if "p2bPrivateOperatorSnapshot" in text and "ACTIVE_PRIVATE_OPERATOR_SNAPSHOT" in text:
            if not re.search(
                r"ACTIVE_PRIVATE_OPERATOR_SNAPSHOT[^=]*=\s*p2bPrivateOperatorSnapshot",
                text,
            ):
                issues.append("adapter ACTIVE snapshot must default to p2b")
    ...
    # P2B verdict + ETH failure reason must appear in frontend source
    frontend_blob = "\n".join(
        p.read_text(encoding="utf-8", errors="replace")
        for p in files
        if p.suffix in {".ts", ".tsx"}
    )
    if "STAGE_4_18P2B_PASS" not in frontend_blob:
        issues.append("frontend source missing STAGE_4_18P2B_PASS verdict display/data")
    if "eth_followup_direction_changed" not in frontend_blob:
        issues.append("frontend source missing eth_followup_direction_changed")
    if "4.18-P2B" not in frontend_blob:
        issues.append("frontend source missing 4.18-P2B stage marker")
...
def main() -> int:
    print("NEXUS UI MVP-3 ETH Confirmation Timeline safety check")
    ...
    issues.extend(check_mvp3_policy(files))
    ...
    print(
        f"PASS: scanned {len(files)} files; P2B sanitized snapshot OK; "
        ...
        "STAGE_4_18P2B_PASS + eth_followup_direction_changed present"
    )
    if REPORT_HINT.is_file():
        print(f"  report: {REPORT_HINT.relative_to(ROOT).as_posix()}")
    if P2A_SNAPSHOT.is_file():
        print("  note: P2A snapshot retained; adapter prefers P2B")
