#!/usr/bin/env python3
"""Run NEXUS V14-L Research Security Red Team (two-pass capable)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V14-L Research Security Red Team")
    parser.add_argument("--pass", dest="pass_number", type=int, default=1, choices=[1, 2])
    parser.add_argument("--no-runtime", action="store_true")
    parser.add_argument("--no-artifact", action="store_true")
    args = parser.parse_args()

    from backend.nexus_research_redteam.redteam import run_research_redteam
