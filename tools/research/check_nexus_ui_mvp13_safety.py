#!/usr/bin/env python3
"""NEXUS UI MVP-13 Private Operator UX polish safety scanner.

Runnable as: python tools/research/check_nexus_ui_mvp13_safety.py
"""
...
HERO = FRONTEND_SRC / "components" / "OperatorConsoleHero.tsx"
STATUS = FRONTEND_SRC / "components" / "StatusBadge.tsx"
NAV = FRONTEND_SRC / "components" / "SidebarNav.tsx"
REPORT_VIEWER = FRONTEND_SRC / "components" / "PrivateReportViewerCard.tsx"
RUNBOOK = FRONTEND_SRC / "components" / "OperatorRunbookCard.tsx"
HEALTH = FRONTEND_SRC / "components" / "CheckpointHealthCard.tsx"
README = ROOT / "frontend" / "README.md"
REPORT_HINT = ROOT / "docs" / "ui" / "NEXUS_UI_MVP13_PRIVATE_OPERATOR_UX_POLISH_REPORT.md"
...
DOC_ALLOW = re.compile(
    ...
    r"no\s+30m|no\s+60m|manual\s+only|no\s+/data|Auto-run:\s*false)",
    re.IGNORECASE,
)
...
def policy(files: list[Path]) -> list[str]:
    ...
    for req in (HERO, STATUS, NAV, REPORT_VIEWER, RUNBOOK, HEALTH, README, REPORT_HINT):
        if not req.is_file():
    ...
    if overview.is_file():
        ...
        for n in ("OperatorConsoleHero", "30m now", "Auto-run", "BLOCKED", "HOLD"):
            if n not in ot:
                issues.append(f"OverviewPage missing: {n}")
    if evidence.is_file():
        et = evidence.read_text(encoding="utf-8", errors="replace")
        for n in ("Evidence Center", "PrivateReportViewerCard", "OperatorRunbookCard", "CheckpointHealthCard"):
            if n not in et:
    ...
    if risk.is_file():
        ...
        for n in ("orders", "mock", "ARM", "production", "btc_auto", "billing/accounts/API keys"):
            if n not in rt:
    ...
    if shadow.is_file():
        ...
        for n in (
            ...
            "shadow used for graduation",
            "require operator approval",
        ):
            if n not in st:
                issues.append(f"ProviderShadowPage missing: {n}")
    if NAV.is_file():
        nt = NAV.read_text(encoding="utf-8", errors="replace")
        if re.search(r'path\s*=\s*["\']/(?:trade|orders)', nt) or re.search(
            r'to:\s*["\']/(?:trade|orders)["\']', nt
        ):
            issues.append("SidebarNav contains trade/orders route")
    if app.is_file():
...
def main() -> int:
    print("NEXUS UI MVP-13 Private Operator UX polish safety check")
    ...
    if uniq:
        ...
        return 1
    print(
        f"PASS: scanned {len(files)} files; console/nav polish; HOLD/BLOCKED; "
