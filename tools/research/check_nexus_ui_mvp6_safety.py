#!/usr/bin/env python3
"""NEXUS UI MVP-6 P2D-R1 Partial Snapshot safety scanner.

Checks sanitized P2D-R1 snapshot wiring under frontend/src (demo/snapshots path —
...
Requires PARTIAL_NO_ETH_WATCH + technical PASS + RuntimeRegressionStatusCard.

Runnable as: python tools/research/check_nexus_ui_mvp6_safety.py
"""
...
SNAPSHOT_FILE = SNAPSHOT_DIR / "p2dR1PrivateOperatorSnapshot.ts"
ADAPTER = FRONTEND_SRC / "demo" / "nexusDataAdapter.ts"
SNAPSHOT_TYPES = FRONTEND_SRC / "types" / "nexusSnapshot.ts"
RUNTIME_CARD = FRONTEND_SRC / "components" / "RuntimeRegressionStatusCard.tsx"
REPORT_HINT = ROOT / "docs" / "ui" / "NEXUS_UI_MVP6_P2D_R1_PARTIAL_SNAPSHOT_REPORT.md"
...
DOC_ALLOW = re.compile(
    ...
    r"not\s+started|blocked|sample insufficient)",
    re.IGNORECASE,
)
...
REQUIRED_SNAPSHOT_MARKERS = [
    ...
    "sampleInsufficientReason",
    "btcValidWatchCount: 1",
    "last tick, no follow-up",
...
def check_snapshot_policy() -> list[str]:
    ...
    for marker in REQUIRED_SNAPSHOT_MARKERS:
        if marker not in text:
            issues.append(f"p2dR1PrivateOperatorSnapshot.ts missing marker: {marker}")
...
def check_mvp6_policy(files: list[Path]) -> list[str]:
    issues: list[str] = []
    issues.extend(check_snapshot_policy())

    for required in (ADAPTER, SNAPSHOT_TYPES, RUNTIME_CARD):
    ...
    if ADAPTER.is_file():
        ...
        if "p2dR1PrivateOperatorSnapshot" not in text:
            issues.append("adapter must import / prefer p2dR1PrivateOperatorSnapshot")
        if not re.search(
            r"ACTIVE_PRIVATE_OPERATOR_SNAPSHOT[^=]*=\s*p2dR1PrivateOperatorSnapshot",
            text,
        ):
            issues.append("adapter ACTIVE snapshot must default to p2dR1")

    if RUNTIME_CARD.is_file():
        text = RUNTIME_CARD.read_text(encoding="utf-8", errors="replace")
        for needle in (
            "RuntimeRegressionStatus",
            "technical_valid",
            "PARTIAL_NO_ETH_WATCH",
            ...
            if needle not in text:
                issues.append(f"RuntimeRegressionStatusCard missing: {needle}")
    ...
    for needle in (
        "STAGE_4_18P2D_R1_PARTIAL_NO_ETH_WATCH",
        "PARTIAL_NO_ETH_WATCH",
        "RuntimeRegressionStatusCard",
        "technical PASS",
        "P2E",
        "4.18-P2D-R1",
    ):
        if needle not in frontend_blob and needle.lower() not in frontend_blob.lower():
            # technical PASS may appear as technicalValid / technical_valid
            if needle == "technical PASS":
                if (
                    "technicalValid" in frontend_blob
                    or "technical_valid" in frontend_blob
                    or "technical PASS" in frontend_blob
                ):
                    continue
...
def main() -> int:
    print("NEXUS UI MVP-6 P2D-R1 Partial Snapshot safety check")
    ...
    issues.extend(check_mvp6_policy(files))
    ...
    print(
        f"PASS: scanned {len(files)} files; P2D-R1 sanitized snapshot OK; "
        "RuntimeRegressionStatusCard OK; "
        "PARTIAL_NO_ETH_WATCH + technical PASS + Stage 4.19 blocked displayed; "
