#!/usr/bin/env python3
"""NEXUS UI MVP-4 P2C Confirmation Prompt Issue safety scanner.

Checks sanitized P2C snapshot wiring under frontend/src (demo/snapshots path —
...
Requires P2C verdict + confirmation_prompt_too_strict + system issue display.

Runnable as: python tools/research/check_nexus_ui_mvp4_safety.py
"""
...
SNAPSHOT_FILE = SNAPSHOT_DIR / "p2cPrivateOperatorSnapshot.ts"
P2B_SNAPSHOT = SNAPSHOT_DIR / "p2cPrivateOperatorSnapshot.ts"
...
REPORT_HINT = ROOT / "docs" / "ui" / "NEXUS_UI_MVP4_P2C_CONFIRMATION_PROMPT_ISSUE_REPORT.md"
...
def check_snapshot_policy() -> list[str]:
    ...
    for marker in REQUIRED_SNAPSHOT_MARKERS:
        if marker not in text:
            issues.append(f"p2cPrivateOperatorSnapshot.ts missing marker: {marker}")
...
def check_mvp4_policy(files: list[Path]) -> list[str]:
    issues: list[str] = []
    issues.extend(check_snapshot_policy())

    for required in (ADAPTER, SNAPSHOT_TYPES, ETH_CARD):
    ...
    if ADAPTER.is_file():
        ...
        if "p2cPrivateOperatorSnapshot" not in text:
            issues.append("adapter must import / prefer p2cPrivateOperatorSnapshot")
        if "demo/snapshots" not in text and "snapshots/p2c" not in text:
            issues.append("adapter must import from demo/snapshots (not src/data)")
        # Prefer P2C over P2B/P2A as active default
        if "p2cPrivateOperatorSnapshot" in text and "ACTIVE_PRIVATE_OPERATOR_SNAPSHOT" in text:
            if not re.search(
                r"ACTIVE_PRIVATE_OPERATOR_SNAPSHOT[^=]*=\s*p2cPrivateOperatorSnapshot",
                text,
            ):
                issues.append("adapter ACTIVE snapshot must default to p2c")
    ...
    if ETH_CARD.is_file():
        ...
        for needle in (
            ...
            "hard_skip",
            "confirmation_prompt_too_strict",
            "P2D confirmation prompt review",
            "DemoDataBadge",
            "SANITIZED",
            "NONE",
        ):
    ...
    # P2C verdict + confirmation_prompt_too_strict must appear in frontend source
    frontend_blob = "\n".join(
        p.read_text(encoding="utf-8", errors="replace")
        for p in files
        if p.suffix in {".ts", ".tsx"}
    )
    if "STAGE_4_18P2C_PASS" not in frontend_blob:
        issues.append("frontend source missing STAGE_4_18P2C_PASS verdict display/data")
    if "confirmation_prompt_too_strict" not in frontend_blob:
        issues.append("frontend source missing confirmation_prompt_too_strict")
    if "SYSTEM ISSUE" not in frontend_blob and "system_issue" not in frontend_blob:
        issues.append("frontend source missing SYSTEM ISSUE / system_issue display")
    if "NOT MARKET REVERSAL" not in frontend_blob and "market_valid" not in frontend_blob:
        issues.append("frontend source missing NOT MARKET REVERSAL / market_valid display")

    if "4.18-P2C" not in frontend_blob:
        issues.append("frontend source missing 4.18-P2C stage marker")
...
def main() -> int:
    print("NEXUS UI MVP-4 P2C Confirmation Prompt Issue safety check")
    ...
    issues.extend(check_mvp4_policy(files))
    ...
    print(
        f"PASS: scanned {len(files)} files; P2C sanitized snapshot OK; "
        "ETH confirmation timeline OK; "
        "no secrets/billing/accounts/trade/ARM/routing-editor/4.19-start; "
        "Private Operator + READ ONLY / NOT INVESTMENT ADVICE OK; "
        "STAGE_4_18P2C_PASS + confirmation_prompt_too_strict present"
    )
    if REPORT_HINT.is_file():
        print(f"  report: {REPORT_HINT.relative_to(ROOT).as_posix()}")
    if P2A_SNAPSHOT.is_file():
        print("  note: P2B/P2A snapshots retained; adapter prefers P2C")
