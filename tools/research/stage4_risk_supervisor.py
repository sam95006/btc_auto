...
class Stage4RiskSupervisor:
    ...
    def evaluate(
        ...
        *,
        ...
        max_lev = int(c.get("max_leverage") or MAX_LEVERAGE)
        ...
        for patch in retrieved_patches:
            ...
            if patch_action in VETO_ACTIONS:
                reason = f"active_patch_{patch_action}:{patch.get('setup_key', '')}"
                return self._veto(reason, conf, size, notes, action="force_skip")
        ...
        if action != "enter" or side == "NONE":
            return RiskSupervisorResult(
                ...
                action="force_skip" if action == "enter" else "approve",
