#!/usr/bin/env python3
"""NEXUS UI MVP-7 P2E Regression Readiness safety scanner.

Checks sanitized P2E snapshot wiring under frontend/src/demo/snapshots/.

Runnable as: python tools/research/check_nexus_ui_mvp7_safety.py
"""
...
SNAPSHOT_FILE = SNAPSHOT_DIR / "p2ePrivateOperatorSnapshot.ts"
ADAPTER = FRONTEND_SRC / "demo" / "nexusDataAdapter.ts"
SNAPSHOT_TYPES = FRONTEND_SRC / "types" / "nexusSnapshot.ts"
READY_CARD = FRONTEND_SRC / "components" / "RegressionReadinessCard.tsx"
REPORT_HINT = ROOT / "docs" / "ui" / "NEXUS_UI_MVP7_P2E_REGRESSION_READINESS_REPORT.md"
...
DOC_ALLOW = re.compile(
    ...
    r"not\s+started|blocked|sample_market_no_edge|no\s+60m)",
    re.IGNORECASE,
)
...
def check_snapshot_policy() -> list[str]:
    ...
    if FORBIDDEN_DATA_DIR.exists():
        ...
        if bad_files:
            ...
            issues.append(f"frontend/src/data/ must not be used — found: {rels}")
    ...
    for marker in REQUIRED_SNAPSHOT_MARKERS:
        if marker not in text:
            issues.append(f"p2ePrivateOperatorSnapshot.ts missing marker: {marker}")
...
def check_mvp7_policy(files: list[Path]) -> list[str]:
    issues: list[str] = []
    issues.extend(check_snapshot_policy())

    for required in (ADAPTER, SNAPSHOT_TYPES, READY_CARD):
    ...
    if ADAPTER.is_file():
        ...
        if "p2ePrivateOperatorSnapshot" not in text:
            issues.append("adapter must import / prefer p2ePrivateOperatorSnapshot")
        if not re.search(
            r"ACTIVE_PRIVATE_OPERATOR_SNAPSHOT[^=]*=\s*p2ePrivateOperatorSnapshot",
            text,
        ):
            issues.append("adapter ACTIVE snapshot must default to p2e")

    if READY_CARD.is_file():
        text = READY_CARD.read_text(encoding="utf-8", errors="replace")
        for needle in (
            "RegressionReadinessStatus",
            "current readiness",
            "sample_market_no_edge",
            ...
            if needle not in text:
                issues.append(f"RegressionReadinessCard missing: {needle}")
    ...
    for needle in (
        "STAGE_4_18P2E_PASS",
        "sample_market_no_edge",
        "RegressionReadinessCard",
        "should_run_60m",
        "wait helper fixed",
        "4.18-P2E",
    ):
        if needle not in frontend_blob and needle.lower() not in frontend_blob.lower():
            if needle == "wait helper fixed":
                if "waitHelperFixed" in frontend_blob or "wait_helper_fixed" in frontend_blob:
                    continue
            if needle == "should_run_60m":
                if "shouldRun60m" in frontend_blob or "should_run_60m" in frontend_blob:
                    continue
            issues.append(f"frontend source missing: {needle}")
    ...
    if shadow.is_file():
        ...
        for needle in ("permanent routing", "p2eSummary", "not supported"):
            if needle not in text and needle.lower() not in text.lower():
    ...
    if membership.is_file():
        ...
        for needle in ("Future only", "customer SaaS not implemented", "No billing"):
            if needle not in text:
...
def main() -> int:
    print("NEXUS UI MVP-7 P2E Regression Readiness safety check")
    ...
    issues.extend(check_mvp7_policy(files))
    ...
    print(
        f"PASS: scanned {len(files)} files; P2E sanitized snapshot OK; "
        "RegressionReadinessCard OK; sample_market_no_edge + no 60m + Stage 4.19 blocked; "
