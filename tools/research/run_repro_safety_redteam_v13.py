#!/usr/bin/env python3
"""Run NEXUS V13-H Reproducibility and Safety Red Team (two-pass capable)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V13-H Repro Safety Red Team")
    parser.add_argument("--pass", dest="pass_number", type=int, default=1, choices=[1, 2])
    parser.add_argument("--no-runtime", action="store_true")
    parser.add_argument("--no-artifact", action="store_true")
    args = parser.parse_args()

    from backend.nexus_autonomy.repro_safety_redteam_v13.redteam import run_repro_safety_redteam
