...
BACKTEST_MIN_WIN_RATE = 0.35 if not REVENUE_GROWTH_MODE else 0.28
BACKTEST_MAX_ALLOWED_RECENT_LOSSES = 4 if not REVENUE_GROWTH_MODE else 5
BACKTEST_MAX_NEGATIVE_AVG_PNL = -2.5 if not REVENUE_GROWTH_MODE else -6.0
# High-confidence entries may pass backtest caution in revenue exploration mode.
BACKTEST_CONFIDENT_OVERRIDE = 0.55 if REVENUE_GROWTH_MODE else 0.68
