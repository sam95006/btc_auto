import { sendStationChat } from "../api_client.js?v=20260520a";
import { refreshNexusState } from "../state_store.js?v=20260520a";
import { buildStationConversation, escapeHtml, normalizeText, translateStation } from "../utils/presentation.js?v=20260520a";
...
async function submitChat(channel, text) {
  const trimmed = String(text || "").trim();
  if (!trimmed) return;
  await sendStationChat(channel, trimmed);
  await refreshNexusState("chat-send");
}
...
export function renderChatDock(root, state, uiState, updateUiState) {
  ...
  root.innerHTML = `
    <div class="chat-dock ${minimized ? "is-minimized" : "is-open"}">
      ${
        minimized
          ...
          : `<section class="game-chat-panel">
               ...
               <div class="game-chat-log">${rows.length ? rows.map(renderLine).join("") : `<p class="game-chat-empty">輸入訊息，站長會回覆。</p>`}</div>
  ...
  root.querySelector("[data-chat-toggle]")?.addEventListener("click", () => updateUiState({ minimized: !minimized }));
  root.querySelectorAll("[data-chat-station]").forEach((btn) => {
    btn.addEventListener("click", () => updateUiState({ activeStation: btn.dataset.chatStation, minimized: false }));
  });
  ...
  form?.addEventListener("submit", async (event) => {
    ...
    try {
      await submitChat(activeStation, value);
  ...
  const log = root.querySelector(".game-chat-log");
