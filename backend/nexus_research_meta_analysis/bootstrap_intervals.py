"""Bootstrap and block-bootstrap interval reports (development only)."""
...
def bootstrap_intervals(
    ...
    *,
    ...
    iid_ci = _ci(iid)
    block_ci = _ci(block)
    iid_sign = sum(1 for v in iid if (v > 0) == (observed > 0)) / max(1, len(iid))
    block_sign = sum(1 for v in block if (v > 0) == (observed > 0)) / max(1, len(block))
    stable = (
    ...
    return {
        "axis": "bootstrap_intervals",
        "observed_mean": observed,
        "n_observations": len(series),
        "replicates": replicates,
        "block_size": block_size,
        "ci_level": ci_level,
        "bootstrap_intervals": {**iid_ci, "sign_agreement": iid_sign},
        "block_bootstrap_intervals": {**block_ci, "sign_agreement": block_sign},
