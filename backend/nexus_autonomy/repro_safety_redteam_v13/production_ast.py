"""V13-H production AST mutation wrapper.

Reuses V11 production AST kill-suite against Private Core security modules.
Platform-blocked mutants are NEVER counted as PASS — they become unresolved
blockers unless detect-killed by oracle observation.
"""
...
def run_v13_production_ast_campaign(root: Path | None = None) -> dict[str, Any]:
    ...
    for r in results:
        ...
        # Platform/env refusal without oracle hole observation is not a kill.
    ...
    # Equivalent on a required-detect mutant is honesty-ok only if not claimed PASS kill.
    required_not_killed = [
        mid
    ...
    # Do not call platform-blocked PASS.
    ...
    return {
        "campaign": "v13_production_ast_mutation",
