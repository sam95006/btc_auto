...
Ported from R4 reviewer tools (tools/review/r4_security_authority/ast_mutator.py)
into Lane G production ownership. No mutmut / cosmic-ray dependency.

Mutants are written only under a temp sandbox — production sources are never
edited in-place.
"""
...
def mutate_write_trap_install_noop(source: str) -> tuple[str, MutantSpec | None]:
    ...
    for name in ("install", "install_traps", "enable", "arm"):
        ...
        if fn is not None:
            ...
            ast.fix_missing_locations(tree)
            return ast.unparse(tree), MutantSpec(
                mutant_id="write_trap_install_noop",
