"""V15-K End-to-End Autonomy Campaign V4 campaign.

Integrates Founder-private DecisionLifecycle closed loop at scale
(candidate_count>=100000, completed_lifecycle_count>=50000) with multi-symbol /
multi-regime coverage and fault injection (provider outage, capture
degradation, partial fills, cancel-replace, clock rollback, disk pressure,
ledger interrupt, snapshot corruption, checkpoint rollback, Reflection/Lesson
interrupt, kill switch, restart, qualification blocks).
...
from backend.nexus_e2e_autonomy_v4.probes import run_focused_scale_probes
from backend.nexus_e2e_autonomy_v4.scale_fast_store import (
...
PASS_STATUS = "NEXUS_V15_K_E2E_AUTONOMY_CAMPAIGN_V4_PASS"
INVALID_PREFIX = "NEXUS_V15_K_E2E_AUTONOMY_CAMPAIGN_V4_INVALID"
SMOKE_ONLY_STATUS = "NEXUS_V15_K_E2E_AUTONOMY_CAMPAIGN_V4_SMOKE_ONLY"
PACKAGE = "NEXUS_V15_K_E2E_AUTONOMY_CAMPAIGN_V4"
FROZEN_SEED = 15_011
TARGET_CANDIDATES = 100_000
TARGET_COMPLETED_LIFECYCLES = 50_000
...
def _run_one(
    ...
    *,
    ...
    prefix = f"v15k-{candidate['replay_index']:05d}"
    ...
    # Capture degradation: incomplete/stale capture absorbed then recovered.
    if fault_tag == "capture_degradation":
        counters["capture_degradation_injected"] = (
            counters.get("capture_degradation_injected", 0) + 1
        )
        try:
            orch.observe(
                candidate_id=str(candidate["candidate_id"]),
                ...
                data_freshness={"age_seconds": 3600.0, "stale": True},
                data_completeness={
                    "ratio": 0.25,
                    "required_fields": ["mid", "spread", "ts", "regime"],
                    "present_fields": ["mid"],
                ...
                evidence_blobs=ev["evidence_blobs"],
                idempotency_key=f"{prefix}-obs-degraded",
            )
        except Exception:  # noqa: BLE001 — expected fail-closed path
            counters["capture_degradation_absorbed"] = (
                counters.get("capture_degradation_absorbed", 0) + 1
            )
    ...
    decide_out = orch.decide(
        ...
        execution_intent={
            "idempotency_key": f"v15kintent:{candidate['candidate_id']}",
            "symbol": candidate["symbol"],
            "side": candidate["side"],
            "order_type": "MARKET",
    ...
    reviewed = orch.review(
        did,
        reflection_id=f"refl_v15k_{candidate['replay_index']:05d}",
...
def run_scaled_closed_loop(
    *,
    ...
    def _new_orch(epoch: int) -> tuple[Any, DecisionLifecycleOrchestrator, Path]:
        ...
        # Scale volume: memory checkpoint/ledger; durability covered by focused probes.
        orch._checkpoints = ScaleMemoryCheckpointStore()  # noqa: SLF001
        orch._ledger = ScaleMemoryLedgerLink()  # noqa: SLF001
        return adapter, orch, epoch_root

    # Rotate epoch roots frequently so NTFS/RAM does not collapse under 100k
    # decision objects (in-process orch state is freed on rotate).
    epoch_rotate_every = max(100, min(500, candidate_count // 200 or 250))
    progress_every = max(1000, candidate_count // 50)
    ...
    def _accumulate_writes() -> None:
        ...
        if orch is not None:
            exchange_writes += int(getattr(orch, "exchange_write_attempt_count", 0) or 0)
    ...
    def _rotate_epoch(*, reason: str) -> None:
        ...
        try:
            ...
            extras["epoch_rmtree_soft_fail"] = int(extras.get("epoch_rmtree_soft_fail", 0)) + 1
    ...
    while i < len(candidates):
        ...
        if i % progress_every == 0 or i == candidate_count:
            print(
                f"[v15k-e2e] closed_loop progress index={i}/{candidate_count} "
                f"completed={completed_count} epoch={epoch}",
                flush=True,
            )
    ...
    return {
        "schema": "v15_k_e2e_scaled_closed_loop",
        "seed": seed,
...
def run_fault_injection_session(
    ...
    *,
    ...
    try:
        ...
        return {
            "schema": "v15_k_e2e_fault_injection_session",
...
def run_v15_e2e_autonomy_campaign(
    *,
    ...
    """Full V15-K campaign: scaled closed loop + fault session + focused probes."""
    _harden_env()
    owns_root = root is None and not keep_root
    work = Path(root) if root is not None else Path(tempfile.mkdtemp(prefix="v15k_e2e_"))
    ...
    # Relative integrity for the requested run size (smoke vs full).
    closed_ok = int(closed["candidate_count"]) == int(candidate_count)
    completed_ok = (
    ...
    # Founder absolute floors — NEVER emit PASS_STATUS below these.
    founder_scale_ok = (
        int(closed["candidate_count"]) >= TARGET_CANDIDATES
        and int(closed["completed_lifecycle_count"]) >= TARGET_COMPLETED_LIFECYCLES
    )
    symbol_floor = TARGET_SYMBOL_COUNT if founder_scale_ok else min(4, len(SYMBOLS))
    universe_ok = (
        int((closed.get("universe") or {}).get("symbol_count") or 0) >= symbol_floor
        and int((closed.get("universe") or {}).get("vol_regime_count") or 0)
        >= min(3, len(VOL_REGIMES))
        and int((closed.get("universe") or {}).get("fixture_universe_size") or 0) >= TARGET_SYMBOL_COUNT
    )
    ...
    # Fault coverage attestation — require probe_pass / real injection flags,
    # never mere key presence (anti false-PASS).
    inj_flags = set(fault.get("injection_flags") or [])
    probes_map = focused.get("probes") or {}

    def _probe_ok(name: str) -> bool:
        return bool(probes_map.get(name, {}).get("probe_pass"))
    ...
    fault_coverage = {
        ...
        "capture_degradation": bool({"stale_market_data", "missing_market_data"} & inj_flags)
        or int(inv_raw.get("capture_degradation_injected", 0) or 0) > 0,
        "partial_fills": "partial_fill_before_crash" in inj_flags
        or int(inv_raw.get("partial_fill_injected", 0) or 0) > 0,
        "cancel_replace": _probe_ok("cancel_replace_probe"),
        "clock_anomaly": _probe_ok("terminal_clock_jump_backward")
        or _probe_ok("terminal_clock_jump_forward"),
        "disk_pressure": "disk_soft_limit" in inj_flags
        or _probe_ok("terminal_disk_hard_limit"),
        "ledger_interrupt": "interrupted_ledger_append" in inj_flags
        or _probe_ok("ledger_corruption_probe"),
        "snapshot_corruption": "snapshot_corruption" in inj_flags
        or _probe_ok("snapshot_corruption_probe"),
        "checkpoint_rollback": _probe_ok("checkpoint_rollback_probe")
        or _probe_ok("snapshot_corruption_probe")
        or "snapshot_corruption" in inj_flags,
        ...
        "kill_switch": _probe_ok("terminal_kill_switch_during_open_position"),
        "restart_recovery": restart_ok and _probe_ok("restart_recovery_probe"),
        "qualification_blocks": _probe_ok("qualification_blocks_probe"),
    ...
    blockers: list[str] = []
    if not closed_ok:
        blockers.append(f"candidate_count_mismatch:{closed['candidate_count']}!={candidate_count}")
    if not completed_ok:
        blockers.append(
            f"completed_lifecycles_below_relative_target:{closed['completed_lifecycle_count']}"
        )
    if not founder_scale_ok:
        blockers.append(
            "smoke_only_below_founder_targets:"
            f"candidates={closed['candidate_count']}<{TARGET_CANDIDATES},"
            f"completed={closed['completed_lifecycle_count']}<{TARGET_COMPLETED_LIFECYCLES}"
        )
    if not universe_ok:
    ...
    # Integrity without founder scale → SMOKE_ONLY (never PASS). Scale fail with
    # other defects → INVALID. Only absolute founder floors may emit PASS_STATUS.
    integrity_blockers = [b for b in blockers if not b.startswith("smoke_only_below_founder_targets:")]
    integrity_ok = not integrity_blockers and hard_ban_ok
    if founder_scale_ok and integrity_ok:
        status = PASS_STATUS
    elif (not founder_scale_ok) and integrity_ok:
        status = (
            f"{SMOKE_ONLY_STATUS}:"
            f"candidates={closed['candidate_count']},"
            f"completed={closed['completed_lifecycle_count']}"
        )
    else:
        status = f"{INVALID_PREFIX}:{','.join(blockers[:3])}"

    # Absolute hard ban: PASS_STATUS forbidden when candidate_count < 100000
    # (and when completed_lifecycle_count < 50000). Defense in depth — must not
    # be bypassable by integrity_ok alone.
    if status == PASS_STATUS and int(closed["candidate_count"]) < TARGET_CANDIDATES:
        status = (
            f"{SMOKE_ONLY_STATUS}:hard_ban_candidate_count_below_floor:"
            f"candidates={closed['candidate_count']}<{TARGET_CANDIDATES}"
        )
        if not any(b.startswith("smoke_only_below_founder_targets:") for b in blockers):
            blockers.append(
                "smoke_only_below_founder_targets:"
                f"candidates={closed['candidate_count']}<{TARGET_CANDIDATES},"
                f"completed={closed['completed_lifecycle_count']}<{TARGET_COMPLETED_LIFECYCLES}"
    ...
    if status == PASS_STATUS and (
        ...
        status = (
            f"{SMOKE_ONLY_STATUS}:hard_ban_below_founder_floor:"
            f"candidates={closed['candidate_count']},"
            f"completed={closed['completed_lifecycle_count']}"
        )
        if not any(b.startswith("smoke_only_below_founder_targets:") for b in blockers):
    ...
    report: dict[str, Any] = {
        ...
        "smoke_only": status.startswith(SMOKE_ONLY_STATUS),
        "integrity_ok": integrity_ok,
        "founder_scale_ok": founder_scale_ok,
        "created_at": _utc(),
        "seed": seed,
