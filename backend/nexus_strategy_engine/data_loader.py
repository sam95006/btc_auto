...
Integrity metrics are computed from the complete partition — never hardcoded.
Prefer research data bundles (data_bundle.py) for V1.1 multi-TF work.
"""
...
from backend.nexus_strategy_engine.data_bundle import compute_partition_integrity
        ...
        for p in sorted(h5.glob("*_15_*.json")):
            try:
                ...
                if isinstance(raw, dict) and "candles" in raw:
                    ...
                    ds = _mk_dataset(str(raw.get("symbol") or p.name.split("_")[0]), "15", candles, str(p))
                    if ds:
                        by_sym[ds.symbol] = ds
                        continue
            except Exception:
