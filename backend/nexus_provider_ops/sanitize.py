"""Sanitize Provider ops payloads — never log or emit secrets."""
...
from backend.nexus_provider_ops.constants import FORBIDDEN_LOG_KEYS
...
def assert_no_secret_keys(payload: dict[str, Any]) -> None:
    ...
    if leaked:
        raise RuntimeError(f"provider_ops_secret_keys:{leaked}")
...
def strip_forbidden_keys(payload: Any) -> Any:
    """Deep-copy dropping forbidden secret-bearing keys entirely."""
    if isinstance(payload, dict):
        out: dict[str, Any] = {}
        for k, v in payload.items():
            if str(k).lower() in FORBIDDEN_LOG_KEYS:
...
def safe_log_fields(event: dict[str, Any]) -> dict[str, Any]:
    """Return a loggable event with secret keys removed and auth headers stripped."""
