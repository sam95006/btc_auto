"""V14-K Closed-Loop Scale V3 campaign.

Integrates Founder-private DecisionLifecycle closed loop at scale
(candidate_count>=50000, completed_lifecycle_count>=25000) with multi-symbol /
...
from backend.nexus_scale_v3.probes import run_focused_scale_probes
...
PASS_STATUS = "NEXUS_V14_K_CLOSED_LOOP_SCALE_V3_PASS"
INVALID_PREFIX = "NEXUS_V14_K_CLOSED_LOOP_SCALE_V3_INVALID"
PACKAGE = "NEXUS_V14_K_CLOSED_LOOP_SCALE_V3"
FROZEN_SEED = 14_011
TARGET_CANDIDATES = 50_000
TARGET_COMPLETED_LIFECYCLES = 25_000
...
def _run_one(
    ...
    *,
    ...
    prefix = f"v14k-{candidate['replay_index']:05d}"
    ...
    decide_out = orch.decide(
        ...
        execution_intent={
            "idempotency_key": f"v14kintent:{candidate['candidate_id']}",
            "symbol": candidate["symbol"],
            "side": candidate["side"],
            "order_type": "MARKET",
    ...
    reviewed = orch.review(
        did,
        reflection_id=f"refl_v14k_{candidate['replay_index']:05d}",
...
def run_fault_injection_session(
    ...
    *,
    ...
    try:
        ...
        return {
            "schema": "v14_k_fault_injection_session",
...
def run_v14_closed_loop_scale_campaign(
    *,
    ...
    work = Path(root) if root is not None else Path(tempfile.mkdtemp(prefix="v14k_closed_loop_"))
    ...
    symbol_floor = TARGET_SYMBOL_COUNT if candidate_count >= TARGET_CANDIDATES else min(4, len(SYMBOLS))
    universe_ok = (
    ...
    fault_coverage = {
        ...
        "clock_anomaly": "terminal_clock_jump_backward" in probe_names,
        ...
        "checkpoint_rollback": "snapshot_corruption" in inj_flags
        or "snapshot_corruption_probe" in probe_names
        or (
            "checkpoint_rollback_probe" in probe_names
            and bool(
                (focused.get("probes") or {}).get("checkpoint_rollback_probe", {}).get("probe_pass")
            )
        ),
