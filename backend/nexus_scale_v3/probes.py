"""Focused probes for V14-K closed-loop scale V3 (cancel-replace, interrupts, restart)."""
...
from backend.nexus_scale_v3.injections import SCALE_TERMINAL_INJECTIONS
    ...
    for flag in SCALE_TERMINAL_INJECTIONS:
        ...
        try:
            ...
            result = orch.run_accelerated_session(
                session_id=f"V14K_T_{flag.upper()}",
                logical_hours=24.0,
                candidates=cands,
                injections=[flag],
                checkpoint_every=8,
...
def run_ledger_corruption_probe(base: Path) -> dict[str, Any]:
    ...
    orch = AutonomousSessionOrchestratorV11(root, max_positions=2, max_intents=2)
    try:
        orch.start("V14K_PROBE_LEDGER", logical_hours=1.0)
    ...
    if ledger.exists():
        ...
        try:
            outcome = recover_from_checkpoint(root, "V14K_PROBE_LEDGER")
...
def run_snapshot_corruption_probe(base: Path) -> dict[str, Any]:
    ...
    orch = AutonomousSessionOrchestratorV11(root, max_positions=2, max_intents=2)
    try:
        orch.start("V14K_PROBE_SNAP", logical_hours=1.0)
    finally:
        orch.close()
    lkg = _lkg_path(root)
    ...
    if lkg.exists():
        ...
        outcome = recover_from_checkpoint(root, "V14K_PROBE_SNAP")
        probe["recovery_status"] = outcome.status
        probe["recovery_reason"] = outcome.reason
        probe["probe_pass"] = (
...
def run_checkpoint_rollback_probe(base: Path) -> dict[str, Any]:
    ...
    orch = AutonomousSessionOrchestratorV11(root, max_positions=2, max_intents=2)
    try:
        orch.start("V14K_PROBE_CKPT_RB", logical_hours=1.0)
    ...
    if lkg.exists():
        ...
        lkg.write_text(json.dumps(pointer) + "\n", encoding="utf-8")
        outcome = recover_from_checkpoint(root, "V14K_PROBE_CKPT_RB")
        probe["recovery_status"] = outcome.status
        probe["recovery_reason"] = outcome.reason
...
def run_focused_scale_probes(base: Path, *, seed: int) -> dict[str, Any]:
    ...
    return {
        "schema": "v14_k_closed_loop_scale_v3_focused_probes",
        "probe_pass": all_pass,
        "probes": probes,
        "invariants": inv,
        "created_at": _utc(),
