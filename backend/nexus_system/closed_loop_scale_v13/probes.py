"""Focused probes for V13-G closed-loop scale (cancel-replace, interrupts, restart)."""
...
from backend.nexus_system.closed_loop_scale_v13.injections import SCALE_TERMINAL_INJECTIONS
from backend.nexus_system.closed_loop_scale_v13.invariants import empty_invariant_counts
    ...
    for flag in SCALE_TERMINAL_INJECTIONS:
        ...
        try:
            ...
            result = orch.run_accelerated_session(
                session_id=f"V13G_T_{flag.upper()}",
                logical_hours=24.0,
                candidates=cands,
                injections=[flag],
                checkpoint_every=8,
...
def run_ledger_corruption_probe(base: Path) -> dict[str, Any]:
    ...
    orch = AutonomousSessionOrchestratorV11(root, max_positions=2, max_intents=2)
    try:
        orch.start("V13G_PROBE_LEDGER", logical_hours=1.0)
    ...
    if ledger.exists():
        ...
        try:
            outcome = recover_from_checkpoint(root, "V13G_PROBE_LEDGER")
...
def run_snapshot_corruption_probe(base: Path) -> dict[str, Any]:
    ...
    orch = AutonomousSessionOrchestratorV11(root, max_positions=2, max_intents=2)
    try:
        orch.start("V13G_PROBE_SNAP", logical_hours=1.0)
    finally:
        orch.close()
    lkg = _lkg_path(root)
    ...
    if lkg.exists():
        ...
        outcome = recover_from_checkpoint(root, "V13G_PROBE_SNAP")
        probe["recovery_status"] = outcome.status
        probe["recovery_reason"] = outcome.reason
        probe["probe_pass"] = (
...
def run_focused_scale_probes(base: Path, *, seed: int) -> dict[str, Any]:
    ...
    return {
        "schema": "v13_g_closed_loop_scale_focused_probes",
        "probe_pass": all_pass,
        "probes": probes,
        "invariants": inv,
        "created_at": _utc(),
