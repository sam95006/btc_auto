...
def assert_checkpoint_not_written(path: str | Path | None = None) -> bool:
    """Guard: lane must not write to the canonical checkpoint path."""
    p = Path(path or CANONICAL_CHECKPOINT_PATH)
    # Soft check — we never open for write in this package.
